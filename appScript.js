function doGet(e) {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (passedBlocks.length > 0) {
        ...
        if (remainingBlocks.length > 0) {
          ...
          // Si tous les blocs ont été déplacés, vider la ligne ou la marquer pour suppression
          updates.push({
            row: i + 1,
            participants: "", // Vider les participants
            dates: "" // Vider les dates
          });
        }
      }
    }
  ...
  function addLog(e) {
    try {
      ...
      // Format de l'heure personnalisé : 13h40m23s
      const heures = Utilities.formatDate(now, Session.getScriptTimeZone(), "HH");
      const minutes = Utilities.formatDate(now, Session.getScriptTimeZone(), "mm");
      const secondes = Utilities.formatDate(now, Session.getScriptTimeZone(), "ss");
      const heure = heures + "h" + minutes + "m" + secondes + "s";
